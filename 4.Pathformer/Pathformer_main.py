...
def main_train_test_model(gene_all,gene_select,pathway_gene_w,pathway_crosstalk_network,data_path,label_path, save_path,
               dataset,model_name,batch_size,gradient_num,epoch_num,patience, delta,stop_epoch,validation_each_epoch_no,
               depth,heads,dim_head,beta, attn_dropout, ff_dropout, classifier_dropout, lr_max,lr_min):
    #######################
    ...
    if label_dim == 2:
        ...
        for epoch in range(1, EPOCH_NUM+1):
            ...
            if (epoch>stop_epoch)&(early_stopping.save_epoch) :
                ...
                if (epoch>stop_epoch)&(early_stopping.early_stop) :
                    break
        ckpt = torch.load(save_path + '/ckpt/'+model_name+'_'+str(early_stopping.best_epoch)+'.pth')
        torch.save(ckpt,save_path + '/ckpt/'+model_name+'_best.pth')
        f.close()
...
def main(args):
    ...
    main_train_test_model(args.gene_all,args.gene_select,args.pathway_gene_w,args.pathway_crosstalk_network,args.data_path,args.label_path, args.save_path,
                   args.dataset,args.model_name,args.batch_size,args.gradient_num,args.epoch_num,args.patience, args.delta,args.stop_epoch,args.validation_each_epoch_no,
