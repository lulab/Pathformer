...
class Embedding_layer(nn.Module):
    def __init__(self, feature_len, embeding):
        ...
        self.M = torch.nn.Parameter(torch.tensor(scale(np.random.rand(feature_len, embeding))).float(),
                                    requires_grad=True)
...
class AxialAttention_2(nn.Module):
    ...
    def forward(self, x, edges=None, mask=None, output_attentions=False):
        ...
        if output_attentions:
            out, attn_out_1, attn_out_2 = self.attn(x, mask=mask, attn_bias=attn_bias, tie_dim=tie_dim,
...
class EvoformerBlock_2(nn.Module):
    def __init__(
        ...
        self.layer = nn.ModuleList([
            AxialAttention_2(dim=row_dim, heads=heads, dim_head=dim_head, dropout=attn_dropout, row_attn=True,
                             col_attn=False, accept_edges=True, norm_tf=norm_tf),
            AxialAttention_2(dim=col_dim, heads=heads, dim_head=dim_head, dropout=attn_dropout, row_attn=False,
                             col_attn=True, norm_tf=norm_tf),
    ...
    def forward(self, inputs, output_attentions=False):
        ...
        if output_attentions:
            ...
            return x[:, :, :, 0], m[:, :, :,0], mask, msa_mask, attn_out_row_1, attn_out_row_2
        else:
...
class Evoformer_2(nn.Module):
    def __init__(
        ...
        self.embedding_layer = Embedding_layer(feature_len=self.col_dim, embeding=self.embeding_num)
        self.layers = nn.ModuleList(
            [EvoformerBlock_2(row_dim=self.row_dim, col_dim=self.col_dim, heads=self.heads, dim_head=self.dim_head,
                              beta=self.beta, norm_tf=self.norm_tf,
    ...
    def forward(self, x, m, mask=None, msa_mask=None, output_attentions=False):
        ...
        for layer in self.layers:
            ...
            if output_attentions:
                ...
                x, m, mask, msa_mask, attn_out_row_1, attn_out_row_2 = layer(inp,output_attentions=output_attentions)
        ...
        if output_attentions:
            ...
            return x, m, attn_out_row_1_list, attn_out_row_2_list
        else:
            return x, m
...
class omics_model(nn.Module):
    def __init__(self,
        ...
        self.Evoformer = Evoformer_2(depth=depth, dim_network=1, row_dim=embeding_num,
    ...
    def forward(self, x, m_row, output_attentions):
        ...
        if output_attentions:
            ...
            x, m, attn_out_row_1_list, attn_out_row_2_list = self.Evoformer(x,m,output_attentions=output_attentions)
        else:
        ...
        if output_attentions:
            ...
            return dec_logits, attn_out_row_1_list, attn_out_row_2_list,x
        else:
            return dec_logits
