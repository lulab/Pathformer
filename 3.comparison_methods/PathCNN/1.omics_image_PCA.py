...
def get_pac(dataset,label_path,data_path,save_path):
    pathway_data=pd.read_csv('/apps/home/lulab_liuxiaofan/qhsky/project/multi_omics_paper/reference/pathway_all.txt',sep='\t')
    label=pd.read_csv(label_path,sep='\t')
    sample_train=list(label.loc[label['dataset_'+str(dataset)+'_test']=='train','sample_id'])
    sample_test=list(label.loc[label['dataset_'+str(dataset)+'_test']=='test','sample_id'])
    sample_validation=list(label.loc[label['dataset_'+str(dataset)+'_test']=='validation','sample_id'])
