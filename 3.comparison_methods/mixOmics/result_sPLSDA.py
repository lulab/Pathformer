...
for c in ['BRCA_subtype', 'BRCA_stage', 'BRCA_survival']:
    ...
    for d in range(1, 11):
        ...
        for t in ['merge']:
            print(t)
            save_path = '/apps/home/lulab_liuxiaofan/qhsky/project/multi_omics_paper/Result/' + c + '/mixOmics/sPLSDA/' + str(d) + '/' + t + '/'
            data = pd.read_csv(save_path + '/result_cv_2_10.csv', sep='\t', header=None)
            ...
            if y_num == 2:
                ...
                for m in [2, 5, 10]:
                    for n in [10, 50, 100, 200, 500]:
                        try:
                            data = pd.read_csv(save_path + '/result_cv_' + str(m) + '_' + str(n) + '_new.csv', sep='\t',header=None)
                            ...
                            data_result['num'] = m
                            data_result['feature_num'] = n
                            data_result_all = pd.concat([data_result_all, data_result])
                        except FileNotFoundError:
                            continue

                data_result_all = data_result_all.astype('float')
                data_result_mean = data_result_all.groupby(['num', 'feature_num']).mean()
                max_num = int(list(data_result_mean.sort_values('roc_auc_Averaged').index)[-1][0])
                max_feature = int(list(data_result_mean.sort_values('roc_auc_Averaged').index)[-1][1])

                data_validation = pd.read_csv(save_path + '/result_validation_' + str(max_num) + '_' + str(max_feature) + '_new.csv', sep='\t',header=None)
                ...
                data_result_all = data_result_all.astype('float')
                data_result_mean = data_result_all.groupby(['num', 'feature_num']).mean()
                max_num = int(list(data_result_mean.sort_values('f1_macro_Averaged').index)[-1][0])
                max_feature = int(list(data_result_mean.sort_values('f1_macro_Averaged').index)[-1][1])
                ...
                acc_Averaged_train, auc_weighted_ovr_Averaged_train, auc_weighted_ovo_Averaged_train, auc_macro_ovr_Averaged_train, auc_macro_ovo_Averaged_train, f1_weighted_Averaged_train, f1_macro_Averaged_train = plot_roc_multi(predict_train_Averaged_new, rankdata(y_train, method='dense') - 1)
                acc_Weighted_train, auc_weighted_ovr_Weighted_train, auc_weighted_ovo_Weighted_train, auc_macro_ovr_Weighted_train, auc_macro_ovo_Weighted_train, f1_weighted_Weighted_train, f1_macro_Weighted_train = plot_roc_multi(predict_train_Weighted_new, rankdata(y_train, method='dense') - 1)
