...
def to_sparse(x):
    ...
    sparse_tensortype = getattr(torch.sparse, x_typename) #返回一个对象属性值
...
def cal_adj_mat_parameter(edge_per_node, data, metric="cosine"):
    ...
    dist = cosine_distance_torch(data, data)  #样本余弦距离矩阵
    parameter = torch.sort(dist.reshape(-1,)).values[edge_per_node*data.shape[0]] #排序后选位置，edge_per_node越大,取值越大
...
def gen_adj_mat_tensor(data, parameter, metric="cosine"):
    ...
    g = graph_from_dist_tensor(dist, parameter, self_dist=True) #小于parameter的变为线
    if metric == "cosine":
        adj = 1-dist
    else:
        raise NotImplementedError
    adj = adj*g 
    adj_T = adj.transpose(0,1) #转置
    I = torch.eye(adj.shape[0])
    if cuda:
        I = I.cuda()
    adj = adj + adj_T*(adj_T > adj).float() - adj*(adj_T > adj).float()
    adj = F.normalize(adj + I, p=1) #将某一个维度除以那个维度对应的范数(默认是2范数)，对行操作
    adj = to_sparse(adj)
    
    return adj


def gen_test_adj_mat_tensor(data, trte_idx_tr,trte_idx_te, parameter, metric="cosine"):
    assert metric == "cosine", "Only cosine distance implemented"
    adj = torch.zeros((data.shape[0], data.shape[0]))
    if cuda:
        adj = adj.cuda()
    num_tr = len(trte_idx_tr)
    
    dist_tr2te = cosine_distance_torch(data[trte_idx_tr], data[trte_idx_te])
    ...
    dist_te2tr = cosine_distance_torch(data[trte_idx_te], data[trte_idx_tr])
