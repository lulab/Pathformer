...
def data_filter(raw_data_path,save_path,cancer):
    print('miRNA')
    ...
    print('DNAmethy')
    data_DNA=pd.read_csv(raw_data_path+cancer+'DNAmethy.csv',sep=',',nrows=1)
    ...
    print('CNV_gene')
    data_CNV_gene=pd.read_csv(raw_data_path+cancer+'.CNV_gene_level.txt',sep='\t',nrows=1)
...
def main(args):
    data_filter(args.raw_data_path, args.save_path, args.cancer)
