...
def get_label(stage_label_path,save_path):
    data=pd.read_csv(stage_label_path,sep='\t')
    ...
    if 'pathologic_stage' in list(data.columns):
        ...
        sample_id_2.to_csv(save_path+'/sample_id_label_2.txt',sep='\t',index=False)
    else:
        print('miss')

    sample_id_2=pd.read_csv(save_path+'/sample_id_label_2.txt',sep='\t')
...
def main(args):
    get_label(args.stage_label_path,args.save_path)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Data preprocessing')
    parser.add_argument('--stage_label_path', type=str, required=True,
                        help='stage_label_path', dest='stage_label_path')
