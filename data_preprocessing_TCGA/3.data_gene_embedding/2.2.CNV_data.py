...
def get_data(rawdata_path,save_path,cancer):
    ...
    for i in sample:
        ...
        j=j+1
    data.to_csv(save_path+'/BRCA.CNV_masked_seg.all.csv',sep='\t',index=False)

    ID_gene=pd.read_csv(save_path+'/CNV_id.txt',sep='\t')
    ...
    data_count.to_csv(save_path+'/CNV_count.txt',sep='\t',index=False)
    del data_
    del data_count
    ...
    data_down_max.to_csv(save_path+'/data_down_max.txt',sep='\t',index=False)
    del data_down_max
    data_down_min=data_down[['gene_id']+sample].groupby('gene_id').apply(lambda g: g.min(skipna=True))
    data_down_min.to_csv(save_path+'/data_down_min.txt',sep='\t',index=False)
    del data_down_min
    ...
    data_down_sum.to_csv(save_path+'/data_down_sum.txt',sep='\t',index=True)
    del data_down_sum
    data_down_mean=data_down[['gene_id']+sample].groupby('gene_id').apply(lambda g: g.mean(skipna=True))
    data_down_mean.to_csv(save_path+'/data_down_mean.txt',sep='\t',index=True)
    del data_down_mean
    del data_down
    del data
