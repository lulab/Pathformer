...
def get_data(rawdata_path,save_path,cancer):
    ...
    ID_gene=pd.read_csv(save_path+'/DNA_methylation_promoterid.txt',sep='\t')
    ...
    data_max.to_csv(save_path+'/promoter_methylation_max.txt',sep='\t',index=False)
    del data_max
    #min
    ...
    data_min.to_csv(save_path+'/promoter_methylation_min.txt',sep='\t',index=False)
    del data_min
    #mean
    data_mean=data[['gene_id']+sample].groupby('gene_id').apply(lambda g: g.mean(skipna=True))
    data_mean=data_mean.fillna(0)
    data_mean=data_mean.reset_index()
    data_mean.to_csv(save_path+'/promoter_methylation_mean.txt',sep='\t',index=False)
    del data_mean
    #count
    data=data.fillna(0)
    ...
    data_count.to_csv(save_path+'/promoter_methylation_count.txt',sep='\t',index=False)
