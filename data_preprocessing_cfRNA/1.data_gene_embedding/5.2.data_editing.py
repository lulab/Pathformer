...
def get_feature(input_path,reference_path,save_path):
    ...
    data=pd.read_csv(input_path+'/editing_rawdata.txt',sep='\t')
    data=data.fillna(0)
    sample=list(data.columns[1:])
    ID=pd.read_csv(save_path+'/editing_ID.txt',sep='\t')
    ...
    data_max.to_csv(save_path+'/editing_max.txt',sep='\t',index=False)
    ...
    data_min.to_csv(save_path+'/editing_min.txt',sep='\t',index=False)

    #mean
    data_mean=data[['gene_id']+sample].groupby('gene_id').apply(lambda g: g.mean(skipna=True))
    data_mean=data_mean.fillna(0)
    data_mean=data_mean.reset_index()
    data_mean.to_csv(save_path+'/editing_mean.txt',sep='\t',index=False)
    ...
    data_count.to_csv(save_path+'/editing_count.txt',sep='\t',index=False)
    ...
    data_entropy.to_csv(save_path+'/editing_entropy.txt',sep='\t',index=False)



    #all
    depth_all=pd.read_csv(input_path+'/editing_totalcount_rawdata.txt',sep='\t')
    depth_all=pd.merge(depth_all,ID[['ID','gene_id']],on='ID',how='left')
    depth_alt=pd.read_csv(input_path+'/editing_altcount_rawdata.txt',sep='\t')
    ...
    ratio.to_csv(save_path+'/editing_mean_all.txt',sep='\t',index=False)
    ...
    ratio_1.to_csv(save_path+'/editing_mean_window_1.txt',sep='\t',index=False)
    ...
    ratio_2.to_csv(save_path+'/editing_mean_window_2.txt',sep='\t',index=False)
    ...
    ratio_3.to_csv(save_path+'/editing_mean_window_3.txt',sep='\t',index=False)
