...
def get_ID_all(input_path,reference_path,save_path):
    ...
    data_ID=pd.concat([data['ID'].map(lambda x:x.split('|')[2]),data['ID'].map(lambda x:x.split('|')[3])])
    data_ID=pd.DataFrame(data_ID)
    data_ID.index=range(len(data_ID))
    data_ID.columns=['ID']
    ...
    data_ID.to_csv(save_path+'/chimeric_ID.txt',sep='\t',index=False)
