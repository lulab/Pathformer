...
def get_feature(input_path,reference_path,save_path):
    ...
    ID=pd.read_csv(save_path+'/SNP_ID.txt',sep='\t')
    ...
    data_max.to_csv(save_path+'/SNP_max.txt',sep='\t',index=False)
    ...
    data_count.to_csv(save_path+'/SNP_count.txt',sep='\t',index=False)
    ...
    data_entropy.to_csv(save_path+'/SNP_entropy.txt',sep='\t',index=False)



    #all
    depth_all=pd.read_csv(input_path+'/SNP_depth_sum.txt',sep='\t')
    depth_all.columns=[i.split('.')[0] for i in list(depth_all.columns)]
    depth_all=pd.merge(depth_all,ID[['ID','gene_id']],on='ID',how='left')
    depth_alt=pd.read_csv(input_path+'/SNP_alt_depth.txt',sep='\t')
    ...
    ratio.to_csv(save_path+'/SNP_mean_all.txt',sep='\t',index=False)
    ...
    def get_location(x):
        ...
        if len(gtf_)==0:
            ...
            try:
                locat=float(x['ID'].split('_')[-3].split('.')[0])
    ...
    ratio_1.to_csv(save_path+'/SNP_mean_window_1.txt',sep='\t',index=False)
    ...
    ratio_2.to_csv(save_path+'/SNP_mean_window_2.txt',sep='\t',index=False)
    ...
    ratio_3.to_csv(save_path+'/SNP_mean_window_3.txt',sep='\t',index=False)
