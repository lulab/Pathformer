...
def get_data(path, reference_path, label_path, save_path):
    feature_data = pd.read_csv(reference_path+'/gene_mRNA_cfRNA.txt', header=None)
    feature_data = list(feature_data[0])
    label = pd.read_csv(label_path, sep='\t')
    sample = list(label['sample_id'])
    feature_type = pd.read_csv(reference_path+'/feature_type_cfRNA.txt', sep='\t', header=None)
    ...
    for i in range(len(feature_type)):
        ...
        if feature_type.loc[i, 'type_name'].split('/')[0] == 'AS':
            data['gene_id'] = data['feature'].map(lambda x: x.split('|')[1].split('.')[0]).astype(str)
        else:
            data['gene_id'] = data['feature'].map(lambda x: x.split('|')[0].split('.')[0]).astype(str)
...
def main(args):
    get_data(args.path, args.reference_path, args.label_path, args.save_path)
