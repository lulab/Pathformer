...
sys.path.append(r'/apps/home/lulab_liuxiaofan/qhsky/project/multi_omics_paper/comparison_methods/MOGAT/')
...
def prepare_trte_data(view_list, data_path, label_path, dataset, data_type):
    num_view = len(view_list)
    label = pd.read_csv(label_path, sep='\t')
    sample_train = label.loc[label['dataset_' + str(dataset) + '_new'] == 'train', 'sample_id']
    ...
    for type in view_list:
        if data_type != 'merge':
            data = pd.read_csv(data_path + '/data_' + type + '_' + data_type + '.txt',sep='\t')
        else:
...
def main_model(label_path, data_path, dataset, save_path, epoch_num, patience, delta,stop_epoch):
    setup_seed(2022)
    ...
    data_tensor_tr_list, data_tensor_val_list, data_tensor_te_list, idx_dict, label_val, label_te, y_num = prepare_trte_data(
        view_list, data_path, label_path, dataset, data_type)
    num_class = len(set(label_te))
    dim_hvcdn = pow(num_class, num_view)
    # train
    labels_tr_tensor = torch.LongTensor(label_te[idx_dict["tr"]])
    # onehot_labels_tr_tensor = one_hot_tensor(labels_tr_tensor, num_class) #one hot编码
    ...
    # onehot_labels_val_tensor = one_hot_tensor(labels_val_tensor, num_class) #one hot编码
    ...
    # onehot_labels_te_tensor = one_hot_tensor(labels_te_tensor, num_class) #one hot编码
    ...
    if y_num == 2:
        ...
        for epoch in range(1, num_epoch + 1):
            ######模型训练########
            tr_prob, loss_dict = train_epoch(data_tensor_tr_list, adj_tr_list, labels_tr_tensor,
                                             sample_weight_tr, model_dict, optim_dict)
            ACC_train, AUC_train, f1_weighted_train, f1_macro_train = get_roc(labels_tr_tensor.data.cpu().numpy(),
                                                                              tr_prob[:, 1])
            # 输出
            ...
            ACC_val, AUC_val, f1_weighted_val, f1_macro_val = get_roc(labels_val_tensor.data.cpu().numpy(),
                                                                      val_prob[:, 1])
            # 输出
            ...
            if epoch % val_inverval == 0:
                te_prob, test_loss = test_epoch(data_tensor_te_list, adj_te_list, labels_te_tensor,
                                                sample_weight_te, idx_dict["te"], model_dict)
                ACC_test, AUC_test, f1_weighted_test, f1_macro_test = get_roc(labels_te_tensor.data.cpu().numpy(),
                                                                              te_prob[:, 1])
                # 输出
        ...
        for epoch in range(1, num_epoch + 1):
            ...
            acc_train, auc_weighted_ovr_train, auc_weighted_ovo_train, auc_macro_ovr_train, auc_macro_ovo_train, f1_weighted_train, f1_macro_train = get_roc_multi(
                labels_tr_tensor.data.cpu().numpy(), tr_prob)
            ...
            acc_val, auc_weighted_ovr_val, auc_weighted_ovo_val, auc_macro_ovr_val, auc_macro_ovo_val, f1_weighted_val, f1_macro_val = get_roc_multi(
                labels_val_tensor.data.cpu().numpy(), val_prob)
            ...
            if epoch % val_inverval == 0:
                ...
                acc_test, auc_weighted_ovr_test, auc_weighted_ovo_test, auc_macro_ovr_test, auc_macro_ovo_test, f1_weighted_test, f1_macro_test = get_roc_multi(
                    labels_te_tensor.data.cpu().numpy(), te_prob)
...
def main(args):
    main_model(args.label_path, args.data_path, args.dataset, args.save_path, args.epoch_num, args.patience, args.delta, args.stop_epoch)
