...
def clf_select(name, pds):
    if name == 'DT':
        ...
        clf = GridSearchCV(LogisticRegression(penalty='l2', solver='liblinear'), tree_para, cv=pds, n_jobs=5,
                           scoring='f1_macro')
    elif name == 'KNN':
        ...
        clf = GridSearchCV(xgb.XGBClassifier(learning_rate=0.5, min_child_weight=3, gamma=3, subsample=0.7,
                                             ...
                                             scale_pos_weight=1, nthread=5), tree_para, cv=pds, n_jobs=5,
                           scoring='f1_macro')
    return clf
...
def clf_select_multi(name, pds):
    if name == 'DT':
        ...
        clf = GridSearchCV(xgb.XGBClassifier(learning_rate=0.5, min_child_weight=3, gamma=3, subsample=0.7,
                                             objective='binary:logistic',
                                             scale_pos_weight=1, nthread=5), tree_para, cv=pds,
                           n_jobs=5, scoring='f1_macro')
    return clf


def main_model(data_path, label_path, feature_name, dataset, method, num, save_path):
    ...
    for i in range(num):
        ...
        if len(set(label['y'])) > 2:
            ...
            acc_train, auc_train, f1_weighted_train, f1_macro_train = get_roc(y_train,predict_train[:, 1])
            acc_test, auc_test, f1_weighted_test, f1_macro_test = get_roc(y_test,predict_test[:,1])
...
def main(args):
    main_model(args.data_path, args.label_path, args.feature_name, args.dataset, args.method, args.num, args.save_path)
