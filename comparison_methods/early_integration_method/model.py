...
def main_model(data_path, label_path, feature_name, dataset, method, num, save_path):
    ...
    sample_discovery = label.loc[label['dataset_' + str(dataset)] == 'discovery', 'sample_id']
    ...
    for i in range(num):
        ...
        if len(set(label['y'])) > 2:
            ...
            acc_discovery, auc_discovery, f1_weighted_discovery, f1_macro_discovery = get_roc(y_discovery, predict_discovery[:, 1])
            acc_validation, auc_validation, f1_weighted_validation, f1_macro_validation = get_roc(y_validation, predict_validation[:, 1])
