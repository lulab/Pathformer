...
#######函数############
######################
class lfNN(torch.nn.Module):
    def __init__(self, in_dim,num_classes):
        super(lfNN, self).__init__()
        self.FC_1 = nn.Sequential(
                    nn.Linear(in_features=in_dim[0], out_features=500),
                    nn.ReLU(),
                    nn.Dropout(0.1),
        ...
        self.FC_2 = nn.Sequential(
                    nn.Linear(in_features=in_dim[1], out_features=500),
                    nn.ReLU(),
                    nn.Dropout(0.1),
                    nn.Linear(in_features=500, out_features=100),
                    nn.ReLU(),
        ...
        self.FC_3 = nn.Sequential(
                    nn.Linear(in_features=in_dim[2], out_features=500),
                    nn.ReLU(),
                    nn.Dropout(0.1),
                    nn.Linear(in_features=500, out_features=100),
                    nn.ReLU(),
    ...
    def forward(self, x,in_dim,num_classes):
        x_1=self.FC_1(x[:,:in_dim[0]])
        x_2 = self.FC_2(x[:,in_dim[0]+1:in_dim[0]+in_dim[1]+1])
        x_3 = self.FC_3(x[:,in_dim[0]+in_dim[1]:in_dim[0]+in_dim[1]+in_dim[2]+1])
...
def main_model(label_path, data_path, dataset, save_path, epoch_num, learning_rate, patience, delta,stop_epoch):
    ...
    model = lfNN(in_dim=[count_feature_len,CNV_feature_len,methylation_feature_len],num_classes=num_y)
