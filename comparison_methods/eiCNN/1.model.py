...
class efCNN(torch.nn.Module):
    def __init__(self, in_dim,num_classes):
        ...
        self.FC = nn.Sequential(
                    ...
                    # nn.ReLU(),
                    # nn.Dropout(0.1),
                    nn.Linear(in_features=int((int((in_dim-1000+1)/100)-50+1)/10)*16, out_features=50),
                    nn.ReLU(),
    ...
    def forward(self, x,num_classes):
        in_dim=x.shape[1]
        x=self.FC(x.unsqueeze(2).permute(0,2,1))
        if num_classes==2:
...
def main_model(label_path, data_path, dataset, save_path, epoch_num, learning_rate, patience, delta,stop_epoch):
    ...
    model = efCNN(in_dim=data.shape[0],num_classes=num_y)
