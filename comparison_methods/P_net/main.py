...
sys.path.append(r'/qhsky1/liuxiaofan_result/model/pnet_pytorch/')
from model.model import get_pnet
...
def main_model(label_path, data_path, dataset, save_path, epoch_num, learning_rate, patience, delta,
               stop_epoch):
    ...
    gene_data = pd.read_csv('/apps/home/lulab_liuxiaofan/qhsky/project/multi_omics_paper/comparison_methods/P_net/data/pathway/Reactome_gene.txt', sep='\t')
    gene_all_data = pd.read_csv('/apps/home/lulab_liuxiaofan/qhsky/project/multi_omics_paper/reference/gene_all.txt', sep='\t',header=None)
    gene_all_data.columns = ['gene_id']
    gene_index = gene_all_data.loc[gene_all_data.gene_id.isin(list(set(gene_data['gene_id']))), :].index
    gene_list = list(gene_all_data.loc[gene_all_data.gene_id.isin(list(set(gene_data['gene_id']))), 'gene_id'])
    gene_data = gene_data.set_index('gene_id')
    gene_name_list = list(gene_data.loc[gene_list, 'gene_name'])
    data_all = np.load(file=data_path + '/data_all.npy')
    data_all = data_all[:, list(gene_index), :]
    data_all = data_all.swapaxes(2, 1).reshape((data_all.shape[0], data_all.shape[1] * data_all.shape[2]))
    data_all = pd.DataFrame(data_all)
    data_all.index = label.index

    data_train = np.array(data_all.loc[train_sample, :])
    data_validation = np.array(data_all.loc[validation_sample, :])
    data_test = np.array(data_all.loc[test_sample, :])
    ...
    genes = pd.Index(gene_name_list)
    n_features = data_train.shape[1]
    n_genes = len(genes)
    model = get_pnet(genes, n_features, n_genes, num_classes=num_y,
                     n_hidden_layers=5,
                     activation=None,
                     ...
                     batch_normal=False, repeated_outcomes=True).to(device)
        ...
        for epoch in range(1, epoch_num+1):
            ...
            for index, (data, labels) in enumerate(train_loader):
                ...
                if index % GRADIENT_ACCUMULATION != 0:
                    ...
                    if type(dec_logits) == list:
                        loss = loss_fn(dec_logits[0], labels)
                        for d in range(1, len(dec_logits)):
                            loss = loss + loss_fn(dec_logits[d], labels)
                    loss.backward()
                if index % GRADIENT_ACCUMULATION == 0:
                    dec_logits = model(data)
                    if type(dec_logits) == list:
                ...
                if (type(dec_logits) == list):
                    prediction_scores = dec_logits[0]
                    for d in range(1, len(dec_logits)):
                        prediction_scores = prediction_scores + dec_logits[d]
                    prediction_scores = prediction_scores / len(dec_logits)
                if (index) * batch_size <= train_label.shape[0]:
                    predict_train[(index - 1) * batch_size:(index) * batch_size,
                    :] = prediction_scores.data.cpu().numpy()
                else:
                    predict_train[(index - 1) * batch_size:, :] = prediction_scores.data.cpu().numpy()
            ...
            with torch.no_grad():
                for index, (data, labels) in enumerate(val_loader):
                    ...
                    if type(logits) == list:
                        ...
                        for d in range(1, len(logits)):
                            loss = loss + loss_fn(logits[d], labels)
                    running_loss += loss.item()
                    y_val.extend(labels.tolist())
                    if (type(logits) == list):
                        prediction_scores = logits[0]
                        for d in range(1, len(logits)):
                            prediction_scores = prediction_scores + logits[d]
                        prediction_scores = prediction_scores / len(logits)
                    if (index) * batch_size <= len(validation_sample):
                        predict_val[(index - 1) * batch_size:(index) * batch_size,
                        :] = prediction_scores.data.cpu().numpy()
                    else:
                        predict_val[(index - 1) * batch_size:, :] = prediction_scores.data.cpu().numpy()
                ...
                with torch.no_grad():
                    for index, (data, labels) in enumerate(test_loader):
                        ...
                        if (index) * batch_size <= len(test_sample):
                            predict_test[(index - 1) * batch_size:(index) * batch_size,
                            :] = prediction_scores.data.cpu().numpy()
                        else:
                            predict_test[(index - 1) * batch_size:, :] = prediction_scores.data.cpu().numpy()
